...
<body>
  ...
  <script>
    ...
    function carregarVisitas() {
      ...
      store.openCursor().onsuccess = e => {
        ...
        if (cursor) {
          ...
          visitas.forEach(v => {
            const iconND = v.layoutND === "sim" ? "👍" : "👎";
            const iconVet = v.layoutVetLife === "sim" ? "👍" : "👎";
            const div = document.createElement("div");
            div.className = "card";
            div.innerHTML = `
              <strong>${v.filial}</strong> <em>(${v.regiao})</em><br/>
              <small>${v.data}</small>
              <p>${v.obs}</p>
              <div class="photo-preview">
                ${v.fotos.map(f => `<img src="${f}" onclick='abrirModal("${f}", ${JSON.stringify(v.fotos)})'>`).join("")}
              </div>
              <div>Layout N&D: ${iconND}<br/>Layout Vet LIFE: ${iconVet}</div>
              <button class="btn-apagar" onclick="apagarVisita(${v.id})">🗑️</button>
            `;
    ...
    function apagarVisita(id) {
      if (confirm("Deseja realmente apagar esta visita?")) {
        const tx = db.transaction("visitas", "readwrite");
        const store = tx.objectStore("visitas");
        store.delete(id);
        tx.oncomplete = carregarVisitas;
    ...
    function abrirModal(fotoInicial, fotos = null) {
      let fotoIndex = 0;
      if (Array.isArray(fotos)) {
        fotoIndex = fotos.indexOf(fotoInicial);
      } else {
        fotos = [fotoInicial];
      }
    ...
    function desenharGrafico(visitas) {
      ...
      grafico = new Chart(ctx, {
        type: 'bar',
        data: { labels: regioes, datasets: [{ label: 'Número de Visitas', data: dados, backgroundColor: backgroundColors, borderRadius: 6 }] },
        options: {
          responsive: true,
    ...
    function limparTudo() {
      if (confirm("Deseja realmente apagar todas as visitas? Esta ação não pode ser desfeita.")) {
        const tx = db.transaction("visitas", "readwrite");
        const store = tx.objectStore("visitas");
